"""
Paper detection module using GPT-4o-mini vision model.

This module handles:
1. Camera capture
2. Paper detection using contour detection and filtering
3. Text extraction and recognition using OpenAI's GPT-4o-mini vision model
"""
...
from src.vision.llm import get_text_from_llm
...
class VisionPaperDetector:
    ...
    def _find_paper_contours(self, frame: np.ndarray) -> List[np.ndarray]:
        ...
        all_contours = contours1 + contours2 + contours3  # type: ignore
        ...
        if not paper_contours:
            logger.debug(
                "No paper contours with standard criteria, trying more permissive filters"
            )
    ...
    def detect_paper(self, frame: np.ndarray) -> Optional[PaperDetection]:
        """Detect paper and extract text using GPT-4o-mini vision model.
        ...
        """
        try:
            # Find paper-like contours (keep this part from the original approach)
            ...
            _, buffer = cv2.imencode(".jpg", paper_roi)
            base64_image = base64.b64encode(buffer).decode("utf-8")

            # Send to the LLM for text detection
            logger.info("Sending to LLM for text detection")
            llm_response = get_text_from_llm(base64_image)

            if llm_response and llm_response.found and llm_response.text:
                text = llm_response.text.lower()
                confidence = 0.9  # High confidence since we're using GPT-4o-mini
                logger.info(
                    f"LLM detected text: '{text}' with confidence {confidence:.2f}"
                )
                return PaperDetection(text=text, confidence=confidence)
            else:
                logger.info("LLM did not detect any valid text")
                return None
